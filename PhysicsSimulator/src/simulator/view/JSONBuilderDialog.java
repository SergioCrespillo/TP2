...
public class JSONBuilderDialog extends JDialog {
	...
	private class JsonTableModel extends AbstractTableModel {
		...
		public void setValueAt(Object o, int rowIndex, int columnIndex) {
			data.get(rowIndex).add(columnIndex, o.toString());
		}
		...
		public String getData() {
			...
			for (int i = 0; i < data.size(); i++) {
				s.append('"');
				s.append(data.get(i).get(0));
				s.append('"');
				s.append(':');
				s.append(data.get(i).get(1));
				s.append('"');
				s.append(':');
				s.append(data.get(i).get(2));
				s.append(',');
			}
	...
	private void initGUI() {
		...
		setTitle("Law Force selection");
		JPanel mainPanel = new JPanel();
		mainPanel.setLayout(new BoxLayout(mainPanel, BoxLayout.Y_AXIS));
		setContentPane(mainPanel);

		// help
		JLabel help = new JLabel("Select a force law and provide values for the parametes in the Value column (default values are used for parametes with no value).");
